...
from libs.dataloader import WinnerDataset
...
def train_epoch(model:SingleOutputModel, dataloader, criterion,
    ...
    for batch_input, batch_truth in pbar:
        model.zero_grad()
        ...
        confusion_matrixs += confusion_matrix(batch_truth, np.argmax(batch_pred, axis=-1), labels=[0,1])
    ...
    with np.printoptions(linewidth=150, formatter={'float': '{:5.03f}'.format, 'int': '{:2} '.format}):
        ...
        print("pred :", np.uint8(np.argmax(batch_pred, axis=-1))[:30])
        print("truth:", batch_truth[:30])
        print("corct:", np.array(["X", " "])[np.uint8(np.argmax(batch_pred, axis=-1)==batch_truth)[:30]])
...
def main(args) -> None:
    ...
    my_train_dataset = WinnerDataset(args.length, train_video_id_list)
    my_valid_dataset = WinnerDataset(args.length, valid_video_id_list)

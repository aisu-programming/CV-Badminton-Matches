...
from libs.pose_keypoints import pose_keypoints
...
def main(video_id, mode, output_video=False):
    ...
    video_output_path = f"data/{mode}/{video_id:05}/{video_id:05}_pose_fixed.mp4"
    csv_output_path = f"data/{mode}/{video_id:05}/{video_id:05}_pose_fixed.csv"
    ...
    with open(csv_output_path, mode='w') as csv_file:
        ...
        for frame_id, current_frame in tqdm(enumerate(video), desc=f"[{mode}] {video_id:05}.mp4: Detecting pose", total=len(video)):

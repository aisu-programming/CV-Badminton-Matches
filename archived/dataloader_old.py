...
class RoundHeadDataset(Dataset):
    def __init__(self, length, video_id_list) -> List[int]:
...
class BackhandDataset(Dataset):
    def __init__(self, length, video_id_list) -> List[int]:
        ...
        hl = (length-1) // 2  # half_length
        self.backhand_count = [ 0 ] * 2

        input_imgs, input_kpts, input_balls, input_times, input_hitters, input_bg_ids, ground_truths = [], [], [], [], [], [], []
        for video_id in tqdm(video_id_list, desc="Preparing BackhandDataset"):

            hit_df_values  = pd.read_csv(f"data/train/{video_id:05}/{video_id:05}_S2.csv")[["HitFrame", "Hitter", "Backhand"]].values
            pose_df_values = pd.read_csv(f"data/train/{video_id:05}/{video_id:05}_pose.csv").values
            ball_df_values = pd.read_csv(f"data/train/{video_id:05}/{video_id:05}_ball_33_adj.csv")[["Adjusted X", "Adjusted Y"]].values
            video_frame_count = len(pose_df_values)

            for hit_frame, hitter, backhand in hit_df_values:
                ...
                input_hitters.append(hitter)
                input_bg_ids.append(bg_id)
                ground_truths.append(backhand)
                self.backhand_count[backhand] += 1
...
class ClassificationDataset(Dataset):
    def __init__(self, length, video_id_list, target) -> List[int]:
        
        hl = (length-1) // 2  # half_length
        self.ground_truth_count = [ 0 ] * 2

        input_imgs, input_kpts, input_balls, input_times, input_hitters, input_bg_ids, ground_truths = [], [], [], [], [], [], []
        for video_id in tqdm(video_id_list, desc=f"Preparing Dataset for {target}"):

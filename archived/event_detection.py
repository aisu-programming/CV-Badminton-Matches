...
with open(filename, newline='') as csvFile:
    ...
    for i in range(1,len(list1)):
        frames.append(int(float(list1[i][0])))
        rx = int(float(list1[i][7])) if list1[i][7]!='' else 0
        ry = int(float(list1[i][8])) if list1[i][8]!='' else 0
        realx.append(rx)
        realy.append(ry)
        if int(float(list1[i][7])) != 0:
            front_zeros[num] = count
            points.append((rx,ry,int(float(list1[i][0]))))
            num += 1
        else:
            count += 1
...
#Sʄy�frame = peaks[i]+int(front_zeros[peaks[i]])
print()
print('Extra points : ')

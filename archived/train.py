...
from libs.dataloader import MyMapDataset, split_datasets
...
def train_epoch(model, dataloader, criterion, optimizer, lr_scheduler, device):
    ...
    for batch_i, (batch_input, batch_truth) in pbar:
        ...
        acc = np.average((batch_pred>0.5)==(batch_truth>0.5))
        ...
        if (batch_i+1) % 50 == 0:
            with np.printoptions(formatter={'float': '{:5.03f}'.format}):
                ...
                # print("\nbatch_truth:", batch_truth[0, :10])
                # print(  "batch_pred :", np.argmax(batch_pred[0], axis=0))[:10]

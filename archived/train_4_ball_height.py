...
from libs.dataloader import BallHeightDataset
...
def main(args) -> None:
    ...
    my_train_dataset = BallHeightDataset(args.length, train_video_id_list)
    my_valid_dataset = BallHeightDataset(args.length, valid_video_id_list)
    ...
    print("backhand_count:", my_train_dataset.backhand_count)
    train_btc_avg      = sum(my_train_dataset.backhand_count) / len(my_train_dataset.backhand_count)
    train_weight       = [ (train_btc_avg/btc) for btc in my_train_dataset.backhand_count ]
    train_weight_torch = torch.from_numpy(np.array(train_weight)).float().to(args.device)
    valid_btc_avg      = sum(my_valid_dataset.backhand_count) / len(my_valid_dataset.backhand_count)
    valid_weight       = [ (valid_btc_avg/btc) for btc in my_valid_dataset.backhand_count ]
...
if __name__ == "__main__":
    ...
    DEFAULT_SAVE_DIR = f"logs/{DEFAULT_MODE}/4_ball_height/{time.strftime('%Y.%m.%d-%H.%M.%S', time.localtime())}"

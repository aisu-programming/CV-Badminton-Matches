...
class MyLinear1D(nn.Module):
    def __init__(self, in_channels, out_channels, act_fn=nn.SiLU(), dropout=0.1) -> None:
        super().__init__()
...
class MyLinear2D(nn.Module):
    def __init__(self, in_channels, out_channels, num_features, act_fn=nn.SiLU(), dropout=0.1) -> None:
        super().__init__()
        self.linear        = nn.Linear(in_channels, out_channels)
...
class ImageProcessor(nn.Module):
    def __init__(self, length) -> None:
        ...
        self.sequential = nn.Sequential(
            # MyConv2d( 64,  64, stride=1),  # (BS,           64, 125, 125) --> (BS,  64, 125,   125)
            MyConv2d( 24,  64),            # (BS, (length+1)*3, 250, 250) --> (BS,  64, 125,   125)
            MyConv2d( 64, 128),            # (BS,           64, 125, 125) --> (BS, 128,  63,    63)
            MyConv2d(128, 256),            # (BS,          128,  63,  63) --> (BS, 256,  32,    32)
            MyConv2d(256, 512),            # (BS,          256,  32,  32) --> (BS, 512,  16,    16)
            MyConv2d(512, 512),            # (BS,          512,  16,  16) --> (BS, 512,   8,     8)
            MyConv2d(512, 512),            # (BS,          512,   8,   8) --> (BS, 512,   4,     4)
            nn.Flatten(),                  # (BS,          512,   4,   4) --> (BS,            8192)
            MyLinear1D(8192,  128),
            MyLinear1D( 128,    1, nn.Sigmoid()),
        )
...
class MyModel(nn.Module):
    ...
    def forward(self, images, datas) -> torch.Tensor:
        ...
        x = self.image_processor(images)      # (BS,         ...) --> (BS, length, 128)
        # x = self.data_processor(datas)        # (BS,         ...) --> (BS, length, 128)

...
def get_player_condition(person, court_yt, court_yb):
    if abs(person["bbox"][1]-court_yt) < abs(person["bbox"][3]-court_yb):
        return 'A'
    else:
        return 'B'
...
def in_court(person, court_values, condition_values):
    ...
    if court_yt < bbox[3] < court_yb:                             # Between yt & yb
        if bbox_x - left_ratio*bbox_y - left_constant > 0:        # On right of left line
            if bbox_x - right_ratio*bbox_y - right_constant < 0:  # On left of right line
                return True
    return False
...
def main(video_id):
    ...
    with open(csv_output_path, mode='w') as csv_file:
        ...
        bboxs, keypoints = [], []
        for frame_id, current_frame in enumerate(mmcv.track_iter_progress(videoReader)):
            ...
            if len(person_results) == 2:
                ...
                player_condition = get_player_condition(person_results[0], court_yt, court_yb)
            ...
            if len(person_results) == 2:
                ...
                if player_condition == 'B': csv_file.write(','*39)
                csv_file.write(f",{bbox[4]},{bbox[0]},{bbox[2]},{bbox[1]},{bbox[3]}")
                for kpt in keypoints: csv_file.write(f",{kpt[0]},{kpt[1]}")
                if player_condition == 'A': csv_file.write(','*39)
            csv_file.write('\n')
